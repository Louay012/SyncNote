...
import { GET_ME, GET_MY_DOCUMENTS, GET_DOCUMENT, SEND_COLLABORATION_INVITE, UNSHARE_DOCUMENT, UPDATE_DOCUMENT, DELETE_DOCUMENT } from "@/lib/graphql";
...
function MyDiariesInternal() {
  ...
  async function handleShare(event) {
    ...
    try {
      ...
      setCollabEmail("");
      await Promise.all([refetchShareDoc(), refetchMine()]);
    } catch (error) {
  ...
  return (
    <AppShell title="My Diaries" subtitle={`Signed in as ${meData?.me?.name || "User"}`}>
      <section className="panel dashboard-toolbar">
        <p className="list-meta">Your diary documents (dated entries).</p>
      </section>
        ...
        <section className="modal-backdrop" role="presentation">
          <article className="panel modal-card" role="dialog" aria-modal="true">
            ...
            <div className="doc-visibility-control">
              <label htmlFor="diaries-mine-visibility-select">Document visibility</label>
              <select
                id="diaries-mine-visibility-select"
