...
// Simple smoke test: two headless Yjs clients connect to the running y-websocket
...
let dbQuery = null;
try {
  const db = await import('../back/src/db/postgres.js');
  dbQuery = db.query;
} catch (e) {
...
async function main() {
  ...
  if (!docId) {
    ...
    console.error('Example: node scripts/test-yjs-smoke.mjs 3');
    process.exit(1);
  }
